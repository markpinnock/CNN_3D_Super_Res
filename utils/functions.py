...
""" Loads images in minbatches """
def imgLoader2(hi_list, lo_list, indices):
    TENSOR_DIMS = 5
    ...
    hi_img = np.expand_dims(np.stack([np.float32(np.load('Hi/' + img)) for img in hi_mb], axis=0), axis=4)
    pre_lo_img = np.expand_dims(np.stack([np.float32(np.load('Lo/' + img)) for img in lo_mb], axis=0), axis=4)
    lo_img = pre_lo_img[:, :, :, 0::4, :]

    if hi_img.shape[3:5] != (12, 1):
        print(hi_img.shape,  file=sys.stderr)
        raise ValueError("hi_mb tensor shape incorrect")
    
    if lo_img.shape[3:5] != (3, 1):
        print(lo_img.shape,  file=sys.stderr)
        raise ValueError("lo_mb tensor shape incorrect")

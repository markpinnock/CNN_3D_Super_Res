...
class UNet2:
    ...
    def __init__(self, image_batch, start_nc):
        ...
        print(up_layer_0)
        up_layer_neg1 = convTransLayer(name='up_conv_layer_neg1', layer_input=up_layer_0, skip_input=tf.convert_to_tensor(np.zeros([1, 512, 512, 6, 8]), dtype=tf.float32), channels=[self.nc[0], self.nc[0]])
        print(up_layer_neg1)
        up_layer_neg2 = convTransLayer(name='up_conv_layer_neg2', layer_input=up_layer_neg1, skip_input=tf.convert_to_tensor(np.zeros([1, 512, 512, 12, 8]), dtype=tf.float32), channels=[self.nc[0], self.nc[0]])
        print(up_layer_neg2)

    ...
    with open(FILE_PATH + report, 'r') as report:
        ...
        for line in lines:
            ...
            if 'training loss' in line:
                ...
                fold_training_losses.append(float(temp_line.split(']')[0]))

            if 'validation loss' in line and 'Summed' not in line:
                temp_line = line.split('[')[1]
                fold_validation_losses.append(float(temp_line.split(']')[0]))

            if 'Summed validation' in line:
                temp_line = line.split('[')[1]
                summed_validation_losses += float(temp_line.split(']')[0])
...
if ind_flag == False:
    ...
    main_ax.legend(('Training loss', 'Validation loss', 'Val loss: {:.2f}'.format(summed_validation_losses / N_val)))
    plt.show()
